(function ($) {
    ...
    //change logo color
    let nav_brand1 = document.querySelector('.nav-brand').childNodes[0];
    let nav_brand2 = nav_brand1.getAttribute('src');

    $window.on('scroll', function () {
        if ($window.scrollTop() > 0) {
            $('.header-area').addClass('sticky');
            nav_brand1.setAttribute('src', 'img/core-img-asc/logo_.png');
        } else {
            $('.header-area').removeClass('sticky');
            nav_brand1.setAttribute('src', 'img/core-img-asc/logo.png');
        }
    });
